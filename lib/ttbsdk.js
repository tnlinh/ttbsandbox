(function () {
  ...
  window.TTB = function (config) {
    ...
    this.vertical = config.vertical || defaults.vertical;
    this.baseURL = config.baseURL || this.setVertical(this.vertical);
  ...
  window.TTB.getSponsorsList = function (payload) {
    ...
    return ttb._ajax(request, 'webservices/get_sponsor_list.json');
  };
  ...
  window.TTB.prototype = {
    ...
    setSponsor: function (vertical) {
      return this.baseURL = this.config.baseURLPattern.replace('{{sponsor}}', vertical);
    },
